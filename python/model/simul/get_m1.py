...
def compute_phi(signal, axis=-1):
    ...
    phi = 0.5 * ( np.angle( np.dot(signal_copy, np.exp(-2.0j * np.arange(length) * dPhi) ) ) % (2 * np.pi) ) 
...
def get_m1_phi_smooth_rates(rates, n_size=gv.n_size, osc=0): 
    ...
    for i_pop in range(rates.shape[1]) : 
        ...
        smooth_rates_pop = circular_convolution(pop_rates, int(pop_rates.shape[-1]*.1) ) # over neurons 
...
def get_avg_m1_phi_smooth_rates(rates, n_size=gv.n_size): 
    ...
    for i_pop in range(2) : 
        ...
        smooth_rates_pop = circular_convolution(pop_rates, int(pop_rates.shape[-1]*.01) ) # over neurons 

...
def get_model(alpha=0, activation=None, input_dim=4, optimizer='adam', loss='binary_crossentropy', metrics=['accuracy']): 
...
def nested_cv(clf, X, y, param_grid, scaling='standard', n_in=10, n_out=10, n_jobs=-1):
    ...
    grid = GridSearchCV(pipe, param_grid=param_grid, cv=cv_inner, n_jobs=None) # gridsearchcv for hyperparam tuning (inner cv loop) 

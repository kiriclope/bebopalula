...
def get_clf(**kwargs):
    ...
    if 'logitnetAlphaIterCV' in kwargs['clf_name']:
        clf = logitnetAlphaIterCV(lbd=kwargs['lbd'], n_alpha=kwargs['n_alpha'], n_lambda=kwargs['n_lambda'], 
                              n_splits=kwargs['inner_splits'], fold_type=kwargs['fold_type'], scoring=kwargs['inner_score'],
        ...
        clf = logitnetCV(lbd=kwargs['lbd'], alpha=kwargs['alpha'], n_lambda=kwargs['n_lambda'], n_splits=kwargs['inner_splits'],
                         standardize=True, fit_intercept=kwargs['fit_intercept'], prescreen=kwargs['prescreen'],
                         fold_type=kwargs['fold_type'], scoring=kwargs['inner_score'],
        ...
        clf = logitnet(lbd=kwargs['lbd'], alpha=kwargs['alpha'], n_lambda=kwargs['n_lambda'],
                       ...
                       standardize=True, fit_intercept=kwargs['fit_intercept'], 
...
def nested_cv(clf, X, y, param_grid, scaling='standard',
    ...
    if fix_hyper==1: 
        grid = inner_cv(clf, param_grid, scaling=scaling, n_in=n_in, fold_type=in_fold, scoring=in_score, n_jobs=-1) # n_jobs=2 
        grid.fit(X, y) 
...
def temp_nested_cv(clf, X_t_train, X_t_test, y, param_grid, scaling='standard',
    ...
    cv_scores = temp_cross_val_score(grid_cv, X_t_train, X_t_test, y, cv=cv_outer, scoring=out_score, n_jobs=n_jobs, error_score='raise') 
...
def outer_cv(clf, X, y, scaling='standard', n_out=10, folds='stratified', outer_score='accuracy', inner_score='deviance', random_state=None, n_jobs=-1, return_clf=0): 
    ...
    cv_scores = cross_val_score(clf, X, y, cv=cv_outer, n_jobs=n_jobs) 
    
    # if return_clf :
    #     cv_scores, cv_clfs = glmnet_cv_loop(clf, X, y, cv=cv_outer, inner_score=inner_score, outer_score=outer_score, return_clf=1) 
    #     return np.nanmean(cv_scores), cv_clfs
    # else:
    #     cv_scores = glmnet_cv_loop(clf, X, y, cv=cv_outer, inner_score=inner_score, outer_score=outer_score, return_clf=0) 

...
def cv_score_lambda(**kwargs): 
    ...
    for i_epochs in range(X_S1.shape[-1]): 
        ...
        clf.random_state = random_state # same random_state for each values of lambda 
        # clf.verbose=1 
        ...
        for i_lambda in range(len(lambdas)): 
            
            clf.lbd = lambdas[i_lambda] 
            ...
            scores.append( score ) 
            print('epoch', i_epochs, 'lambda', lambdas[i_lambda], 'score', score) 
        ...
        plt.axvline(max_lambda, ls='dashed')

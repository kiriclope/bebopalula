...
figtitle = 'stp_vars'
fig = plt.figure(figtitle, figsize=(1.25*1.618*1.5*5, 1.618*1.25)) 

for i in range(5):
    ax = fig.add_subplot(1, 5, i+1) 
    
    i_neuron = np.random.randint(0, int(u_stp.shape[1]/2) ) 
    plt.plot(u_time, u_stp[:,i_neuron], 'r', label='u') 
    plt.plot(x_time, x_stp[:,i_neuron], 'b', label='x')
    
    plt.plot(x_time, u_stp[:,i_neuron] * x_stp[:,i_neuron], 'k', label='ux') 

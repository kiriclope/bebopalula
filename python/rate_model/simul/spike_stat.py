...
n_neurons = gv.n_pop*gv.n_size 
with pgb.tqdm_joblib( pgb.tqdm(desc='CV', total=n_neurons) ) as progress_bar: 
    CV = Parallel(n_jobs=-1)(delayed(parloop_CV)(spike_times, neurons_id, i_neuron) for i_neuron in range(n_neurons) ) 

with pgb.tqdm_joblib( pgb.tqdm(desc='CV2', total=n_neurons) ) as progress_bar: 
    CV2 = Parallel(n_jobs=-1)(delayed(parloop_CV2)(spike_times, neurons_id, i_neuron) for i_neuron in range(n_neurons) ) 
        
figtitle = 'spike_statistics' 
fig = plt.figure(figtitle, figsize=(1.25*1.618*1.5*2, 1.618*1.25)) 

ax = fig.add_subplot(int('121'))
plt.hist(CV[:gv.n_size], histtype='step', ls='-', color='r') 
plt.hist(CV[gv.n_size:], histtype='step', ls='-', color='b') 

plt.xlabel('CV') 
plt.ylabel('Count') 

ax = fig.add_subplot(int('122'))
plt.hist(CV2[:gv.n_size], histtype='step', ls='-', color='r') 
plt.hist(CV2[gv.n_size:], histtype='step', ls='-', color='b') 

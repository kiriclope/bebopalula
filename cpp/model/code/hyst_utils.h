...
void hyst_init() { 
  ...
  if(HYST_J_EE==1) { 
    if(IF_LIF) 
      J_scaled[0] = GAIN / sqrt_Ka[0] / TAU_SYN[0] * (Vth-Vr) * HYST_X_MIN ; 
    if(IF_BIN) 
  ...
  if(HYST_J_EE==-1) { 
    if(IF_LIF) 
      J_scaled[0] = GAIN / sqrt_Ka[0] / TAU_SYN[0] * (Vth-Vr) * HYST_X_MAX ; 
    if(IF_BIN) 
  ...
  if(HYST_M0==1) { 
    ...
    for(i=0;i<n_pop;i++) { 
      m0 = HYST_X_MIN ; 
      if(IF_LIF) 
	ext_inputs_scaled[i] = ext_inputs[i] * sqrt_Ka[0] * (Vth-Vr) * m0 ; 
      if(IF_BIN) 
...
void hyst_update() { 
  ...
  if(HYST_J_EE!=0) {
    ...
    if(IF_LIF) 
      J_scaled[0] = GAIN * J[0] / sqrt_Ka[0] / TAU_SYN[0] * (Vth-Vr) ;

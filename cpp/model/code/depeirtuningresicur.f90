!    E->I' Facilitating   I'->I depressing  E-E depressing
	...
	parameter(nu=1) 
	parameter (kk=300) 
        parameter(nc=9100,nce=9100,nci=2500)
...
integer, allocatable :: mee(:,:),mei(:,:),mii(:,:) 
...
real*8 gg(-n:n)
	...
	real*8 gee,geeampa,gei,gii,gie,ie,ii   
	real*8 ieon,ieoff,iion,iioff
	real*8 g1ee,g1ei,g1ii,g1ie    
...
allocate (mee(ne,nce),mei(ni,nce),mii(ni,nci))
allocate (mie(ne,nci))
...
write(32,*) sngl(i*360.d0/ni),sngl(100.d0*firi(i)),sngl(cvi(i)),sngl(cv2i(i))
	...
	real*8 prob(0:ne/2)
	integer mee(ne,nce),mei(ni,nce),mii(ni,nci)   
	integer mie(ne,nci)
	integer ncmjj(n),ij
	character*2 typcon
...
!	---------  RUTINA QUE CREA LAS MATRICES DE CONEXION CON COSENO --------
...
!	---- gcos: Distribucion tipo 1+cos(x) para prob. de conexion de ie ----
	dpi=2.d0*acos(-1.d0)
!	-----------------------------------------------------------------------
	do i=1,n1
		theta1(i)=dpi*float(i)/float(n1)

...
C    E->I' Facilitating   I'->I depressing  E-E depressing
	...
	parameter(nu=1) 
	parameter (kk=300) 
        parameter(nc=5200,nce=5200,nci=2000)
	...
	real*8 xx,yy,zz
	real*8 averhe(n),averhi(n)
	real*8 averhe2(n),averhi2(n)
	real*8 averexcie(n),averexcie2(n)
	real*8 averinhibe(n),averinhibe2(n)
	real*8 averexcii(n),averexcii2(n)
	real*8 averinhibi(n),averinhibi2(n)
	real*8 seed2
...
c
	open(2,file='depeirtuningresicur.ini')
	read(2,*) iseed1
c	read(2,*) iseed2
	read(2,*) seed2
	read(2,*) itmax
...
c                       iesqk=iesqk2
c                 iesqk=(-iesqkoff+iesqk0)*(it-it4)/float(it5-it4)
c    .             +iesqkoff
			iesqk=iesqkoff
c                       iisqk=iesqk2/sqrt(kkee)*sqrt(kkii)
c                       iisqk=0.65*sqrt(kkii)
c                 iisqk=(-iisqkoff+iisqk0)*(it-it4)/float(it5-it4)
c    .  	   +iisqkoff
			iisqk=iisqkoff
...
c	write(26,*) (it+(ireal-1)*itmax)*dtf*0.01,
...
c	write(27,*) (it+(ireal-1)*itmax)*dtf*0.01,
...
c		sim(k)=sim(k)+eps*(1-dt2/taufac)
	...
	hipop=hipop/float(ni)
     ...
     .sngl(ue(iu)*nu),sngl(ui(iu)*nu),sngl(iesqk),sngl(iisqk)
	   write(36,*) sngl(it*dtf*0.01d0),
     .sngl(hee(1)*gee+iesqk),sngl(hei(1)*gei),sngl(he(1)+ies)
	   write(37,*) sngl(it*dtf*0.01d0),sngl(hepop),sngl(hipop),
     .sngl(hepop+hipop)
	   write(38,*) sngl(it*dtf*0.01d0),
     .sngl(hee(6)*gee+iesqk),sngl(hei(6)*gei),sngl(he(6)+ies)
